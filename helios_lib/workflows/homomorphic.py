...
class EncryptedVote(HeliosObject):
    ...
    def __init__(self):
        ...
        super(EncryptedVote, self).__init__()
    ...
    def fromElectionAndAnswers(cls, election, answers):
        ...
        return_val.election_uuid = election.uid
    ...
    def fromJSONDict(cls, d, pk=None):
        ev = cls()
        # @todo: election_hash, election_uid
...
class Tally(HeliosObject):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if election:
            ...
            self.tally = [[0 for a in range(q['answers'])] for q in self.questions]
        else:
            self.questions = None
    ...
    def add_vote(self, encrypted_vote, verify_p=True, cumulative_weight=None):
        # do we verify?
        if verify_p:
        ...
        for question_num in range(len(self.questions)):
            ...
            for answer_num in range(answers):
                # It's for election with cumulative voting method
                if type(cumulative_weight) == list:
                    weight = cumulative_weight[question_num][answer_num]
                # It's for election with weighted voting method
                elif type(cumulative_weight) == int:
                    weight = cumulative_weight
                # It's for election with simple voting method
                else:
                    weight = 1
                ...
                # In cumulative voting method, for non selected answers
                weight = weight or 1
                weight_count += weight
                self.tally[question_num][answer_num] = self.add_weight(
                    encrypted_vote.encrypted_answers[question_num].choices[answer_num], weight) \
                                                       * self.tally[question_num][answer_num]

        self.num_tallied += weight_count
    ...
    @staticmethod
    def aggregate_tallies(election, tallies):
        """
        Args:
            election: Helios Election Object
            tallies: List of Tally objects
        ...
        for t in tallies:
            for question_num in range(len(election.questions)):
                ...
                answers = question['answers']
                for answer_num in range(answers):
                    tally.tally[question_num][answer_num] = t[question_num][answer_num] * tally.tally[question_num][
                        answer_num]
        return tally
