...
class HeliosObject(object):
    ...
    def __init__(self, **kwargs):
        ...
        if 'uid' in self.FIELDS and (not hasattr(self, 'uid') or self.uid == None):
    ...
    def __eq__(self, other):
        ...
        return other != None and self.uid == other.uid
...
class EncryptedAnswer(HeliosObject):
    ...
    def fromElectionAndAnswer(cls, election, question_num, answer_indexes):
        ...
        choices = [None for a in range(answers)]
        individual_proofs = [None for a in range(answers)]
        overall_proof = None
        randomness = [None for a in range(answers)]
        ...
        for answer_num in range(answers):
            plaintext_index = 0
...
class EncryptedVote(HeliosObject):
    ...
    An encrypted ballot
    """
    FIELDS = ['encrypted_answers', 'election_hash', 'election_uid']
    ...
    def verify(self, election):
        ...
        # check ID
        if self.election_uid != election.uid:
            return False
    ...
    def toJSONDict(self, with_randomness=False):
        return {
            ...
            'election_uid': self.election_uid
        }
    ...
    def fromJSONDict(cls, d, pk=None):
        ...
        ev.election_uid = d['election_uid']
    ...
    def fromElectionAndAnswers(cls, election, answers):
        ...
        encrypted_answers = [EncryptedAnswer.fromElectionAndAnswer(election, answer_num, answers[answer_num]) for
                             answer_num in range(answers)]
        return cls(encrypted_answers=encrypted_answers, election_hash=election.hash, election_uid=election.uid)
...
class Election(HeliosObject):
    FIELDS = ['uid', 'questions', 'name', 'short_name', 'description', 'voters_hash', 'openreg',
    ...
    JSON_FIELDS = ['uid', 'questions', 'name', 'short_name', 'description', 'voters_hash', 'openreg',
...
class Voter(HeliosObject):
    ...
    """
    FIELDS = ['election_uid', 'uid', 'voter_type', 'voter_id', 'name', 'alias']
    JSON_FIELDS = ['election_uid', 'uid', 'voter_type', 'voter_id_hash', 'name']

    # alternative, for when the voter is aliased
    ALIASED_VOTER_JSON_FIELDS = ['election_uid', 'uid', 'alias']
...
class Trustee(HeliosObject):
    ...
    a trustee
    """
    FIELDS = ['uid', 'public_key', 'public_key_hash', 'pok', 'decryption_factors', 'decryption_proofs', 'email']
...
class CastVote(HeliosObject):
    ...
    """
    FIELDS = ['vote', 'cast_at', 'voter_uid', 'voter_hash', 'vote_hash']
    ...
    def issues(self, election):
        ...
        # check the election
        if self.vote.election_uid != election.uid:
            issues.append("the vote's election uid does not match the election for which this vote is being cast")
